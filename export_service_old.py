...
class ExportService:
    ...
    def export_data(self, request: ExportRequest) -> Dict[str, Any]:
        ...
        if request.include_metrics:
            ...
            if request.channel_name:
                metrics_filters.append(ChannelMetrics.channel_name == request.channel_name)
            
            channel_metrics = self.db.query(ChannelMetrics).filter(and_(*metrics_filters)).all()
            export_data["channel_metrics"] = [self._format_channel_metrics(metric) for metric in channel_metrics]
            
            # Export user metrics
            user_metrics = self.db.query(UserMetrics).filter(and_(*metrics_filters)).all()
